...
export default {
    ...
    this.getInvHeadInfo(params)
      .then(response => {
        ...
        if (this.getInvHeadInfoData.TypeId == 1) {
          this.crrentTypeId = "1";
          //企业信息
          for (var p in this.enterprise1) {
            this.enterprise1[p] = this.getInvHeadInfoData[p];
          }
        } else {
          this.crrentTypeId = "2";
          //个人信息
          for (var p in this.enterprise1) {
            this.enterprise2[p] = this.getInvHeadInfoData[p];
          }
        }
      })
      .catch(() => {
  ...
  methods: {
    ...
    save(value) {
      ...
      if (value == 1) {
        ...
        let params = {
          ...
          InvHeadId :this.$route.query.fapiaoId,//发票id
    ...
    titleCheckShowListFn() {
      // 过滤是企业信息 还是个人信息  houtaiTotalData 只存储企业信息
      this.tempInvHeadTotalData.map(item => {
        if (item.TypeId == 1) {
          this.houtaiTotalData.push(item);
        }
      });
      if (this.houtaiTotalData.length != 0) {
        //点击抬头 边上的加号 弹出list菜单 数据在加载页面就已经请求 只显示前5个
