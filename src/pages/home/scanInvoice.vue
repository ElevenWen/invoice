<template>
    <div>
        ...
        <headerCom :titleScoped="'扫码开票'"></headerCom>    
        <div class="top">
            请选择发票种类
        </div>
        <div class="choice">
                ...
                <ul class="baseInfo"   v-if="crrentTypeId == 1">
                    <li> 
                      <div class="box">
                            ...
                            <input type="text" placeholder="请填写企业名称" @input.stop="titleAddShowListFn('2') "    maxlength="30" @blur.stop="verifyC1()"    v-model="enterprise1.Name">
                            <span class="addIcon" @click.stop="titleAddShowListFn('1')"></span> 
                      </div> 
                    </li>
                    <li> 
                        <div class="box">
            ...
            <ul class="otherInfoMoreShow" v-if="moreShow">
                ...
                <li>
                   <div class="box">
                        <span class="hiddenSpan">o</span>
                        <span>账号</span>
                        <input type="text" placeholder="请填写银行账号" v-model="enterprise1.Account" @input.stop="verifyAccount($event)"   @blur.stop="verifyAccountFn"  maxlength="24">
                   </div>
                </li>
                <li>
                    <div class="box">
...
export default {
  ...
  created() {
    console.log("获取商户的fapiaoId", this.$route.query.fapiaoId);

    // let params = {
    //   Id: this.$route.query.fapiaoId
    // };
    // this.getInvHeadInfo(params)
    //   .then(response => {
    //     // console.log(response, "成功");
    //     this.crrentTypeId = this.getInvHeadInfoData.TypeId;
    //     // 第一次获取的数据一定是齐全的.
    //     this.crrentTypeId = "1";
    //     if (this.getInvHeadInfoData.TypeId == 1) {
    //       //企业信息
    //     } else {
    //       this.crrentTypeId = "2";
    //       //个人信息
    ...
    //   })
    //   .catch(() => {
    //     this.$vux.toast.text("网络故障请刷新后重试", "middle");
    //   });
  ...
  methods: {
    ...mapActions(["tianyancha"]),
    ...mapActions("home", ["getInvHeadInfo", "applyInvoice", "getInvHeadlist"]),
    ...
    save(value) {
      ...
      if (value == 1) {
        ...
        //编辑发票抬头 后传递给后台
        let params = {
          BusinessId: this.$route.query.fapiaoId, //	 商家用户Id 	 string 	 不可
          WxCusId: JSON.parse(sessionStorage.getItem("customerInfo")).WxId, // 申请开票人Id 	 string 	 不可
          InvHeadId: this.enterprise1.InvHeadId, // 发票抬头Id 	 string 	 可
        ...
        //   BusinessId 	 商家用户Id 	 string 	 不可
        //WxCusId 	 申请开票人Id 	 string 	 不可
        //InvHeadId 	 发票抬头Id 	 string 	 可
        // data.WxId 	  	 微信用户ID
        //  data.NickName 	  	 昵称
        ...
        let params = {
          BusinessId: this.$route.query.fapiaoId, //	 商家用户Id 	 string 	 不可
          WxCusId: JSON.parse(sessionStorage.getItem("customerInfo")).WxId, // 申请开票人Id 	 string 	 不可
          InvHeadId: this.enterprise2.InvHeadId, // 发票抬头Id 	 string 	 可
    ...
    titleAddShowListFn(value) {
      // 过滤是企业信息 还是个人信息  houtaiTotalData 只存储企业信息
       console.log('点击ADD',value);
      if(timer) { clearTimeout(timer)} //防抖

      timer =  setTimeout(()=>{ 
         //当用户输入后重置发票id
         this.enterprise1.InvHeadId = '';
         ...
         if (!this.enterprise1.Name.trim() || this.enterprise1.Name.length < 2) {
          ...
          this.getInvHeadlist(params)
            .then(() => { 
              ...
              if (value == 1 && this.getInvHeadlistData.list.length != 0) {
                //点击抬头 边上的加号 弹出list菜单 数据在加载页面就已经请求 只显示前5个
                this.titleCheckShow1 = true;
              } 
              else if (value == 1 && this.getInvHeadlistData.list.length == 0) {
                this.$vux.toast.text('暂无数据', 'middle')  
              }
              if (value == 2 && this.getInvHeadlistData.list.length != 0) {
                //点击抬头 边上的加号 弹出list菜单 数据在加载页面就已经请求 只显示前5个
                this.titleCheckShow2 = true;
              } 
              else if (value == 2 && this.getInvHeadlistData.list.length == 0) {
                this.titleCheckShow2 = false;
                this.$vux.toast.text('暂无数据', 'middle') 
              } 
              console.log(this.titleCheckShow1, this.titleCheckShow2);
            })
            .catch(err => {
              console.log(err);
            });
    ...
    checkCompany(value) {
      ...
      console.log('从天眼查获取数据获取数据',value);
      this.titleCheckShow2 = false;
      //再次精确匹配
      this.enterprise1.InvHeadId = value.Id == undefined ? "" : value.Id;
    ...
    houtaiCheckCompany(data) {
      //从后台获取数据
       console.log('从后台获取数据',data);
      this.enterprise1 = {
        InvHeadId:data.Id,//发票Id
    ...
    verifyC1(value) {
      console.log("valuevaluevaluevalue", this.enterprise1.Name);
      // 抬头名称
      // if (!this.enterprise1.Name) {
      //   return false;
      // }
    ...
    verifyAddress() {
      ...
      tool.regularJudgement("companyAddress", this.enterprise1.Address);
    },
    verifyTel() {
      //验证固定电话
